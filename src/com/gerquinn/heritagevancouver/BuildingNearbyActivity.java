...
public class BuildingNearbyActivity extends Activity implements
	...
	public void buildingListSetup() {
		...
		for (BuildingConstructors wh : westHastings) {
			...
			String log = "Id: " + wh.getID() + " ,Building Name: "
					+ wh.getBuildingName() + ", Address: " + wh.getAddress();
	...
	private void initilizeMap() {
		...
		for (int i = 0; i < building_name_array.size(); i++) {
			...
			distanceBetween = checkNearby(latitude, longitude,
					latitude_array.get(i), longitude_array.get(i));
			System.out.println("Distance Between: " + distanceBetween);

			if (distanceBetween <= 3) {
				// Create marker
				MarkerOptions marker = new MarkerOptions().position(
						new LatLng(latitude_array.get(i), longitude_array
								.get(i))).title(building_name_array.get(i));
